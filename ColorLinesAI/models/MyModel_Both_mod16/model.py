...
class DentlyNet(nn.Module):
    def __init__(self, input_x=0, output_channels=14, is_training=True, batch_size=1):
        ...
        self.mp2d1 = torch.nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2), padding=(0, 0)) # 160

        self.down1_conv = nn.Conv2d(6, 6, kernel_size=(7, 5), stride=(1, 1), padding=(3, 2)) #160 9X7X5X24 = 7560
        self.down1_conv_bn = nn.BatchNorm2d(6, track_running_stats=False, momentum=0.5)

        self.mp2d2 = torch.nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2), padding=(0, 0)) # 80

        self.down2_conv = nn.Conv2d(6, 12, kernel_size=(7, 5), stride=(1, 1), padding=(3, 2))  #24X31X9X24 = 160704
        self.down2_conv_bn = nn.BatchNorm2d(12, track_running_stats=False, momentum=0.5)

        self.mp2d3 = torch.nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2), padding=(0, 0)) #40

        self.down3_conv = nn.Conv2d(12, 24, kernel_size=(9, 3), stride=(1, 1), padding=(4, 1)) #24X3X3X96 = 20736
        self.down3_conv_bn = nn.BatchNorm2d(24, track_running_stats=False, momentum=0.5)

        self.down4_conv = nn.Conv2d(24, 24, kernel_size=(9, 3), stride=(1, 1), padding=(4, 1)) #96X1X1X24 = 2304
        self.down4_conv_bn = nn.BatchNorm2d(24, track_running_stats=False, momentum=0.5)

        self.down5_conv = nn.Conv2d(24, 14, kernel_size=(9, 3), stride=(1, 1), padding=(4, 1)) #24X7X5X9 = 7560
        self.down5_conv_bn = nn.BatchNorm2d(14, track_running_stats=False, momentum=0.5)

        self.down6_conv = nn.Conv2d(14, 14, kernel_size=(9, 3), stride=(1, 1), padding=(4, 1)) #24X7X5X9 = 7560
        self.down6_conv_bn = nn.BatchNorm2d(14, track_running_stats=False, momentum=0.5)

        self.Unmp2d1 = nn.Upsample(scale_factor=16, mode='nearest')
    ...
    def forward(self, x=torch.rand(1, 3, 640, 640), requires_grad=True):
        ...
        skeleton_pred = (x1 - torch.mean(x1)) / torch.sqrt(torch.var(x1) + 0.0000001)
