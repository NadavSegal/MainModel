...
class DentlyNet(nn.Module):
    def __init__(self, maxIdx=49, maxSeg=3):
        super(DentlyNet, self).__init__()
        output_channels = maxIdx + maxSeg
        self.maxIdx = maxIdx
        self.maxSeg = maxSeg
        ...
        self.down6_conv = nn.Conv2d(104, output_channels, kernel_size=(3,3), stride=(1, 1), padding=1)
        self.bn6 = nn.BatchNorm2d(output_channels, track_running_stats=False, momentum=0.5)
        self.down7_conv = nn.Conv2d(3, 9, kernel_size=(3,3), stride=(1, 1), padding=1)
        self.down8_conv = nn.Conv2d(9, 3, kernel_size=(5,5), stride=(1, 1), padding=2)
        self.bn6 = nn.BatchNorm2d(output_channels, track_running_stats=False, momentum=0.5)
    ...
    def forward(self, x=torch.rand(1, 3, 640, 640), requires_grad=True):
        ...
        x = F.relu(self.bn5(self.down5_conv(x)))
        x = torch.cat((x, x1), 1)
        x = F.relu(self.bn6(self.down6_conv(x)))
        
        
        indexing_pred= x.narrow(1, 0, self.maxIdx)
        seg_pred = x.narrow(1, self.maxIdx, self.maxSeg)
        seg_pred = self.mp2d(seg_pred)
        seg_pred = F.relu(self.down7_conv(seg_pred))
        seg_pred = self.down8_conv(seg_pred)
    ...
    #        x = torch.from_numpy(x)
        with torch.no_grad(): 
            ...
            seg_pred = x.narrow(1, self.maxIdx, self.maxSeg)
            seg_pred = self.mp2d(seg_pred)
            seg_pred = self.down7_conv(seg_pred)
            seg_pred = self.down8_conv(seg_pred)
        
            
            indexing_output, indexing_argmax, seg_output, seg_argmax = self.eval_postprocess(indexing_pred, seg_pred)
            
            return indexing_output, indexing_argmax, seg_output, seg_argmax, []
        
    def eval_postprocess(self, indexing_pred, seg_pred):
    ...
    def skeleton_postprocess(self, process_indexing, indexing_argmax):
