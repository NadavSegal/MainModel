  ...
  def draw_tracks(self, out, tracks):
    ...
    for track in tracks[:]:
      ...
      for i in range(N-1):
        ...
        cv2.line(out, p1, p2, (255, 0, 0), thickness=stroke, lineType=16)
        # Draw end points of each track.
        if i == N-2:
          clr2 = (0, 0, 0)
          cv2.circle(out, p2, stroke, clr2, -1, lineType=16)
          list_of_points.append([p2[0]/w, p2[1]/h])
...
class VideoStreamer(object):
  ...
  def next_frame(self):
    ...
    if self.camera:
      ...
      input_image1 = input_image1.astype('float')/255.0
    else:
      image_file = self.listing[self.i]
      input_image, input_image1= self.read_image(image_file, self.sizer)
    # Increment internal counter.
    self.i = self.i + 1
    input_image = input_image.astype('float32')
    return   input_image, input_image1, True, image_file




if __name__ == '__main__':
  root_file_path = 'F:/Records2NightRun/toothBrush/amitai/180819/1/cam2UndistortedNew/'
  root_output_path = 'F:/Records2NightRun/toothBrush/amitai/180819/1/cam2UndistortNew_csv/'

  # Parse command line arguments.
  parser = argparse.ArgumentParser(description='Demo.')
  parser.add_argument('--input', type=str, default=root_file_path,
  ...
  parser.add_argument('--nms_dist', type=int, default=6,
      help='Non Maximum Suppression (NMS) distance (default: 4).')
  parser.add_argument('--conf_thresh', type=float, default=0.0075,
  ...
  print('==> Running Demo.')
  csv_columns = ['image_name','x','y']
  dict_data = []
  csv_root_file = "./csv_cam2Undistorted/"
  ...
  while True:
    ...
    # Get a new image.
    img, input_image, status, f_name = vs.next_frame()
    f_name = f_name.split("/")[-1].split(".")[0] + ".csv"
    ...
    key_points = tracker.draw_tracks(input_image, tracks)
    np.savetxt(root_output_path + f_name, key_points, delimiter=",")
    ...
    if heatmap is not None:
      min_conf = 0.5
      heatmap[heatmap < min_conf] = 0.00001
    ...
    if not opt.no_display:
      input_image = cv2.resize(input_image, (opt.display_scale*opt.W, opt.display_scale*opt.H))
    ...
    if True:
      out_file = os.path.join('/Users/alontetro/Downloads/SuperPointPretrainedNetwork-master/vid/', 'frame_%05d.png' % vs.i)
