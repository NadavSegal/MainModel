...
import torch
import dentlyset 
import dently_model as dently_model_640
import dently_model_t_1280 as dently_model_1280
import dently_model_t_1280 as dently_model_1280rot90
...
class Eval:
    def __init__(self, mode="640"):
        if mode == '640':
            self.model  = dently_model_640.DentlyNet()
        elif mode == '1280':
            self.model = dently_model_1280.DentlyNet()
        elif mode == '1280rot90':
            self.model = dently_model_1280.DentlyNet()
        else:
            print("unsupported mode: {0}".format(mode))
...
#                self.model.load_state_dict(model_dict)
    ...
    def from_img_for_socket(self, real_image):
        if self.mode == "1280":
            temp1 = np.zeros((1280, 1280, 3))
            s1 = 240
            temp1[:, s1:s1 + 800, :] += real_image.copy()
            real_image = temp1
        if self.mode == "1280rot90":
            temp1 = np.zeros((1280, 1280, 3))
            s1 = 240
            temp1[s1:s1 + 800,: , :] += real_image.copy()
            real_image = temp1
        ...
        indexing_output, indexing_argmax, seg_output, seg_argmax, _ = self.model.forward_eval(x)
