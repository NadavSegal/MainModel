...
class data_loader: 
    def __init__(self, train_teeth_list_file='/home/alon/dently_7_19/F__Records__ProtoTypeV2__Invitro__S_12.6.2__Tooth10_WithoutBlood_z/'):
        self.src_inputs_path = get_images_and_labels_path(train_teeth_list_file)
    ...
    def get_random_sample(self):
        ...
        name =  c_path.split("/")[-1][2:]
        
#        print(c_path)
        return [load_image( c_path + ".png"), load_image(c_path + "_min_indRef.png"), load_mat_file_as_np(c_path[:-8] + name + "_skelSubP.mat") ,c_path]
...
#            t, b, l, r = int(s/2) -128, int(s/2) + 128, int(s/2) -128, int(s/2) + 128  
    ...
    def get_sample_for_train(self,
                             aug_function_list=[aug_flip , increase_brightness, random_shifting, down_up],
                             mode= "640"):# 
                     
        
        x, y_src, subP_s, c_path = self.get_random_sample()
        y_src = y_src[:, :, 0]
        if mode == "1280":
            temp = np.zeros((1280,1280))
            temp1 = np.zeros((1280,1280, 3))
            s1 = np.random.randint(0,630)
            s2 = np.random.randint(0,630)
            
            temp[s1:s1 + 640, s2:s2+640] += y_src.copy()
            temp1[s1:s1 + 640, s2:s2+640, :] += x.copy()
            x = temp1
            y_src = temp
#        y_src[y_src % 2 == 1] = 1
#        x = cv2.cvtColor(x, cv2.COLOR_BGR2RGB)
        ...
        dk_l = Skeleton(torch.from_numpy(y_src))
        

        return x, y_src.copy(), y_src, yy, c_path, dk_l
  
#    
#idx_label = load_image('/Users/alontetro/Downloads/D__TempData__70ea14c0-9a84-4cd2-8ee9-ba4edf69f6a7_extGT_min_indRef/00000068_min_indRef.png')
#idx_label = idx_label[:,:,0]
#label = load_mat_file_as_np('/Users/alontetro/Downloads/D__TempData__70ea14c0-9a84-4cd2-8ee9-ba4edf69f6a7_extGT_matFiles (1)/000137_skelSubP.mat')
#all_data = data_loader(['/Users/alontetro/Downloads/D__TempData__70ea14c0-9a84-4cd2-8ee9-ba4edf69f6a7_extGT/'])  
#
