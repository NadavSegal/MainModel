...
class test_summary:
    def __init__(self, test_config):
        ...
        self.test_folders_list=test_config['test_folders_list']
        self.test_data = dentlyset.data_loader(test_config['test_folders_list'])  
        self.eval_model = Dently_eval.Eval()
        self.eval_model.load_Weights(test_config['weights'])
        self.summary_dict = {}
        
        
    def start(self, max_steps_per_folder=5):
        i = 0
        for root_path in self.test_folders_list:
            rp = root_path.split("/")[-2]
            test_data = dentlyset.data_loader([root_path])  
            self.summary_dict[rp] = {}

            for path in test_data.src_inputs_path[:max_steps_per_folder]:
                print(i)
                path1 = path.replace('\\', '/')
                i += 1
                c_path = path1.split(".png")[0]
                path_t = c_path.split("/")[-1][:]
                path = c_path.split("/")[-1][2:]
                if not c_path in self.summary_dict[rp]:
                    self.summary_dict[rp][path_t] = {}
...
#                x, y, out_sub_label = padding_resize(x, y)
    ...
    #            torch.cuda.synchronize()
                ...
                a11 = torch.from_numpy(skeleton1)
                a22 = torch.from_numpy(src_skeleton)
                pred_skl = Skeleton(label=a11)
                label_skl = Skeleton(label=a22)
                ...
                cv2.imwrite(self.test_config['log_folder'] +'{}.png'.format(path_t),skeleton )
                correct_pixels, wrong_pixels, tested_pixels, accuracy = get_matrix_accuracy(skeleton2, src_skeleton)
                self.summary_dict[rp]['titles'] = ['image_path',
                                 'idxAI_eqq_idxGT', 
                                 'err_mad', 
                                 'err_mean',
                                 'correct_pixels', 
                ...
                self.summary_dict[rp][path_t] = [c_path,
...
#        for rp in self.summary_dict:
#            with open('./static/{}.csv'.format(rp), mode='w') as file:
#                file_writer = csv.writer(file, delimiter=',')
#                
#                titels = []
#                for title in self.summary_dict[rp]['titles']:
#                    titels.append(title.ljust(15))
#                  
#                file_writer.writerow(titels)
#                
#                for path in self.summary_dict[rp]:  
#                    if not path == 'titles':
#                        line = []
#                        for data in self.summary_dict[rp][path]:
#                            line.append(data.ljust(15))
#                        
#                        
...
#                file.close()
        ...
        for rp in self.summary_dict:
            with open(self.test_config['log_folder'] + '{}_1.csv'.format(rp), 'w', newline='') as csvfile:    
                ...
                for path in self.summary_dict[rp]:  
                    if not path == 'titles':                        
                        self.summary_dict[rp][path][0]
                        writer.writerow({'image_path': self.summary_dict[rp][path][0], 
                                         'idxAI_eqq_idxGT': self.summary_dict[rp][path][1], 
                                         'err_mad': self.summary_dict[rp][path][2], 
                                         'err_mean': self.summary_dict[rp][path][3], 
                                         'correct_pixels': self.summary_dict[rp][path][4], 
                                         'wrong_pixels': self.summary_dict[rp][path][5], 
                                         'tested_pixels': self.summary_dict[rp][path][6], 
                                         'accuracy': self.summary_dict[rp][path][7], 
                                         t_str: self.summary_dict[rp][path][8],})
