...
class DentlyNet(nn.Module):
    def __init__(self, input_x=0, output_channels=14, is_training=True, batch_size=1):
        ...
        self.down1_conv_bn = nn.BatchNorm2d(14, track_running_stats=False, momentum=0.5)

        self.mp2d1 = torch.nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2), padding=(0, 0), return_indices=True)  # 160

        self.down2_conv = nn.Conv2d(14, 14, kernel_size=(5, 3), stride=(1, 1), padding=(2, 1))  # 24X31X9X24 = 160704
        self.down2_conv_bn = nn.BatchNorm2d(14, track_running_stats=False, momentum=0.5)

        self.mp2d2 = torch.nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2), padding=(0, 0), return_indices=True)  # 80

        self.down3_conv = nn.Conv2d(14, 14, kernel_size=(5, 3), stride=(1, 1), padding=(2, 1))  # 24X3X3X96 = 20736
        self.down3_conv_bn = nn.BatchNorm2d(14, track_running_stats=False, momentum=0.5)

        self.mp2d3 = torch.nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2), padding=(0, 0), return_indices=True)  # 40

        self.down4_conv = nn.Conv2d(14, 42, kernel_size=(11, 7), stride=(1, 1), padding=(5, 3))  # 96X1X1X24 = 2304
        self.down4_conv_bn = nn.BatchNorm2d(42, track_running_stats=False, momentum=0.5)

        self.down5_conv = nn.Conv2d(42, 14, kernel_size=(11, 7), stride=(1, 1), padding=(5, 3))  # 24X7X5X9 = 7560
        self.down5_conv_bn = nn.BatchNorm2d(14, track_running_stats=False, momentum=0.5)

        self.Unmp2d3 = torch.nn.MaxUnpool2d(kernel_size=(2, 2), stride=(2, 2), padding=(0, 0))

        self.down6_conv = nn.Conv2d(14, 14, kernel_size=(5, 3), stride=(1, 1), padding=(2, 1))  # 80 #24X7X5X9 = 7560
        self.down6_conv_bn = nn.BatchNorm2d(14, track_running_stats=False, momentum=0.5)

        self.Unmp2d2 = torch.nn.MaxUnpool2d(kernel_size=(2, 2), stride=(2, 2), padding=(0, 0))

        self.down7_conv = nn.Conv2d(14, 14, kernel_size=(5, 3), stride=(1, 1), padding=(2, 1))  # 160 #24X7X5X9 = 7560
        self.down7_conv_bn = nn.BatchNorm2d(14, track_running_stats=False, momentum=0.5)

        self.Unmp2d1 = torch.nn.MaxUnpool2d(kernel_size=(2, 2), stride=(2, 2), padding=(0, 0))

        self.down8_conv = nn.Conv2d(14, 14, kernel_size=(5, 3), stride=(1, 1), padding=(2, 1))  # 320 #24X7X5X9 = 7560
        self.down8_conv_bn = nn.BatchNorm2d(14, track_running_stats=False, momentum=0.5)
        ...
        self.down0_conv_bnSkel = nn.BatchNorm2d(3, track_running_stats=False, momentum=0.5)

        self.down1_convSkel = nn.Conv2d(3, 3, kernel_size=(15, 7), stride=(1, 1), padding=(7, 3))
        self.down1_conv_bnSkel = nn.BatchNorm2d(3, track_running_stats=False, momentum=0.5)

        self.down3_convSkel = nn.Conv2d(3, 5, kernel_size=(15, 7), stride=(1, 1), padding=(7, 3))
        self.down3_conv_bnSkel = nn.BatchNorm2d(5, track_running_stats=False, momentum=0.5)

        self.down2_convSkel = nn.Conv2d(5, 1, kernel_size=(15, 9), stride=(1, 1), padding=(7, 4))
        self.down2_conv_bnSkel = nn.BatchNorm2d(1, track_running_stats=False, momentum=0.5)
    ...
    def forward(self, x=torch.rand(1, 3, 640, 640), requires_grad=True):
        ...
        x_ind = self.Unmp2d3(x_ind, index3)

        x_ind = self.down6_conv(x_ind)
        x_ind = self.down6_conv_bn(x_ind)
        x_ind = F.leaky_relu(x_ind)

        x_ind = self.Unmp2d2(x_ind, index2)

        x_ind = self.down7_conv(x_ind)
        x_ind = self.down7_conv_bn(x_ind)
        x_ind = F.leaky_relu(x_ind)

        x_ind = self.Unmp2d1(x_ind, index1)

        x_ind = self.down8_conv(x_ind)
        x_ind = self.down8_conv_bn(x_ind)
        x_ind = F.leaky_relu(x_ind)

        x_ind = self.Unmp2d0(x_ind)
