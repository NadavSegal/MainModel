...
class DentlyNet(nn.Module):
    def __init__(self, input_x=0, output_channels=49+3, is_training=True, batch_size=1):
        ...
        self.down0_conv = nn.Conv2d(3, 9, kernel_size=(31, 9), stride=(1, 1), padding=(15, 4)) #320
        ...
        self.down1_conv = nn.Conv2d(9, 24, kernel_size=(7, 5), stride=(1, 1), padding=(3, 2)) #160
        self.down1_conv_bn = nn.BatchNorm2d(24, track_running_stats=False, momentum=0.5)

        #self.mp2d = torch.nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2), padding=(0, 0), return_indices=True)

        self.down2_conv = nn.Conv2d(24, 24, kernel_size=(31, 9), stride=(1, 1), padding=(15, 4))
        self.down2_conv_bn = nn.BatchNorm2d(24, track_running_stats=False, momentum=0.5)

        self.down2_1_conv = nn.Conv2d(24, 96, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        self.down2_1_conv_bn = nn.BatchNorm2d(96, track_running_stats=False, momentum=0.5)

        self.down2_2_conv = nn.Conv2d(96, 24, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0))
        self.down2_2_conv_bn = nn.BatchNorm2d(24, track_running_stats=False, momentum=0.5)

        self.down4_conv = nn.Conv2d(24, 9, kernel_size=(7, 5), stride=(1, 1), padding=(3, 2))
        ...
        self.down3_conv = nn.Conv2d(9, 14, kernel_size=(5, 3), stride=(1, 1), padding=(2, 1)) #320
        self.down3_conv_bn = nn.BatchNorm2d(14, track_running_stats=False, momentum=0.5)

        self.down5_conv = nn.Conv2d(14, 14, kernel_size=(5, 3), stride=(1, 1), padding=(2, 1))
        self.down5_conv_bn = nn.BatchNorm2d(14, track_running_stats=False, momentum=0.5)

        #self.down4_conv = nn.Conv2d(24, 14, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
    ...
    def forward(self, x=torch.rand(1, 3, 640, 640), requires_grad=True):
        ...
        x = F.interpolate(x, size=320)

        x = self.down0_conv(x)
        x = self.down0_conv_bn(x)
        x = F.leaky_relu(x)

        x, index = self.mp2d(x)

        x = self.down1_conv(x)
        x = self.down1_conv_bn(x)
        x = F.leaky_relu(x)

        x = self.down2_conv(x)
        x = self.down2_conv_bn(x)
        x = F.leaky_relu(x)

        x = self.down2_1_conv(x)
        x = self.down2_1_conv_bn(x)
        x = F.leaky_relu(x)

        x = self.down2_2_conv(x)
        x = self.down2_2_conv_bn(x)
        x = F.leaky_relu(x)

        x = self.down4_conv(x)
        x = self.down4_conv_bn(x)
        x = F.leaky_relu(x)

        x = self.Unmp2d(x, index)

        x = self.down3_conv(x)
        x = self.down3_conv_bn(x)
        x = F.leaky_relu(x)

        x = self.down5_conv(x)
        x = self.down5_conv_bn(x)
        x = F.leaky_relu(x)
        ...
        #x8 = x7.narrow(1, -1, -1)
        #x8 = x8.narrow(1, 1, 0)
        # skeleton_pred = torch.softmax(x, dim=1)
        # skeleton_final = torch.argmax(x, dim=1)+1

        indexing_pred = x.narrow(1, 0, 14)  # [:,:49,:,:]
        indexing_pred = torch.exp(F.log_softmax(indexing_pred, 1))
        indexing_most_likely, indexing_argmax = indexing_pred.max(dim=1)

        return indexing_most_likely, indexing_pred, indexing_argmax
